...
public class TableController {
    ...
    @ResponseBody
    public AuthFilter.LayuiTableData ArbitraryTableSQL(String sqlType, String sql) {
         if(Objects.equals(sqlType, "查询")) {
             List<HashMap<String, Object>> table = tableService.selectSQL(sql);
            return AuthFilter.LayuiTableData.layTypeData(table.size(), table, sqlType);
        }
        else if(Objects.equals(sqlType, "新增")) {
            Integer table = tableService.insertSQL(sql);
            return AuthFilter.LayuiTableData.layTypeData(Collections.singletonList(table).size(), Collections.singletonList(table), sqlType);
        }
        else if(Objects.equals(sqlType, "修改")) {
            Integer test1 = tableService.updateSQL(sql);
            return AuthFilter.LayuiTableData.layTypeData(Collections.singletonList(test1).size(), Collections.singletonList(test1), sqlType);
        }
    ...
    public AuthFilter.LayuiTableData arbitraryTableSQL(@RequestBody String arbitraryStr) {
